...
## [2026-08-12] update | 12.1.0 promoted to live retail

`client-retail` moved from the `retail-12-0-7` epoch to `retail-12-1-0` after WoW 12.1.0 shipped to live (build `12.1.0.69273`, 2026-08-11), along with the `retail_interface_matches_current_live_build` assertion (`120007` → `120100`) and the `BLIZZARD_UI_TAG` Docker default (`12.0.7` → `12.1.0`). Because the 12.1 API deltas were gated on the epoch feature rather than on `client-ptr`, the promotion was otherwise a one-line change — the gating discipline in `systems/client-profiles.md` paid off exactly as intended.

Promoting the epoch activated the previously-inactive `retail_client_can_point_at_patch_12_1_api_epoch` test and exposed one real bug: `C_UnitAuras.TriggerPrivateAuraShowDispelType` was hidden with a plain `= nil`, but `C_UnitAuras` has an `__index` metatable, so the key was cleared from the raw table while normal lookups still resolved a function through the fallback. The 12.1-removed symbol therefore stayed addon-visible. Fixed by applying the same removed-key metatable filter `src/ptr/strict_removals.lua` uses, and covered by a new regression test that asserts against an index read rather than `rawget`. See `investigations/patch-12-1-api-audit.md`.
